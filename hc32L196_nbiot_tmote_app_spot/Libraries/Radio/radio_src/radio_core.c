...
volatile u8 radio_Send_num = 0;
volatile u8 radio_Rest_num = 0;
...
u8 radio_core_StartTx_Variable_Packet(u8 channel, u8* pioRadioPacket, u8 length)
{
	...
	if (length >= RADIO_TX_ALMOST_EMPTY_THRESHOLD) {
		...
		radio_Rest_num = length + 1 - RADIO_TX_ALMOST_EMPTY_THRESHOLD;
	}
	else {
...
/**********************************************************************************************************
 ...
 @Description			radio_core_StartTx_dummy				: Set Radio to TX mode
 @Input				channel							: Freq. Channel
 @Return				void
**********************************************************************************************************/
...
/**********************************************************************************************************
 @Function			void radio_core_StartTx(u8 channel, u8* pioRadioPacket, u8 len)
 @Description			radio_core_StartTx					: start send long package data
 @Input				channel							: Freq. Channel
					len								: Packet to be sent length of of the packet sent to TXFIFO
 @Return				void
**********************************************************************************************************/
...
void radio_core_Sleep(void)
{
	...
	radio_rf_state = RF_STATE_SLEEP;
}

/**********************************************************************************************************
 @Function			char radio_core_Get_State(void)
