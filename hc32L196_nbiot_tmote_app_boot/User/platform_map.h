...
typedef struct _system_env_data
{
	u32						MacSN;
	u8						BootVersion;
	u8						BootMode;
...
} systemEnvDataTypeDef;

extern systemEnvDataTypeDef systemEnvData;
...
void TCFG_EEPROM_Set_VenderCode(u32 vender);																			//保存Vender Code
u32  TCFG_EEPROM_Get_VenderCode(void);																				//读取Vender Code
...
void TCFG_EEPROM_Set_BootCount(u16 bootCount);																		//保存BootCount
u16  TCFG_EEPROM_Get_BootCount(void);																				//读取BootCount
